document.addEventListener('DOMContentLoaded', () => {
  displaySafeFoods();
  displayAvoidFoods();
  displaySafeCatFoods(); // 추가된 함수: 고양이 음식 초기 표시
  displayAvoidCatFoods(); // 추가된 함수: 고양이 금기 음식 초기 표시
});
...
function displaySafeFoods() {
  ...
  safeFoods.forEach(food => {
    ...
    listItem.classList.add('safe-food'); // 먹어도 되는 음식을 나타내는 클래스 추가
...
function displayAvoidFoods() {
  ...
  avoidFoods.forEach(food => {
    const listItem = document.createElement('li');
    listItem.textContent = food;
    listItem.classList.add('avoid-food'); // 금기 음식을 나타내는 클래스 추가
...
function displaySafeCatFoods() {
  ...
  safeCatFoods.forEach(food => {
    const listItem = document.createElement('li');
    listItem.textContent = food;
    listItem.classList.add('safe-cat-food'); // 먹어도 되는 고양이 음식을 나타내는 클래스 추가
    safeCatFoodsList.appendChild(listItem);
  });
}

// 추가된 함수: 고양이 금기 음식 초기 표시
function displayAvoidCatFoods() {
  ...
  avoidCatFoods.forEach(food => {
    const listItem = document.createElement('li');
    listItem.textContent = food;
    listItem.classList.add('avoid-cat-food'); // 금기 음식을 나타내는 클래스 추가
